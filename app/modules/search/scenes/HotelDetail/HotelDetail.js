...
class HotelDetail extends React.Component {
    ...
    ratingCompleted(rating) {
      const userlikesRef = database.ref('rating').child(`${this.props.hotelkey}/${this.props.user.uid}/`);
      const commentsRef = database.ref('rating').child(`${this.props.hotelkey}/ratingscount`);
    ...
    componentDidMount() {
      database.ref('rating').child(`${this.props.hotelkey}/${this.props.user.uid}/`).on('value', (snap) =>{
        snap.forEach((child) => {
          this.setState({
        rating: child.val(),
          })
      ...
      database.ref('rating').child(`${this.props.hotelkey}`).on('value', (snap) =>{
        let total = 0;
        let user = 0;
        let rating = 0;
          snap.forEach((child) => {
      ...
      database.ref('comments').child(`${this.props.hotelkey}`).on('value', (snaps) => {
        var item = [];
        var commentkey = [];
...
<Text h4 style={{marginTop:15,alignSelf:'center'}}> {this.props.hotelname} </Text>
{this.props.hotelstate ? <Text  style={{marginTop:2,alignSelf:'center',fontSize:16}}> {this.props.hotelstate},{this.props.hotelcountry} </Text> : null}
{this.props.hotelcontactno ? <Text  style={{marginTop:2,alignSelf:'center',fontSize:12}}>Contact No: {this.props.hotelcontactno} </Text> : null}
...
{this.props.hotelreview ? <Text  style={{marginTop:2,alignSelf:'center',fontSize:16,marginLeft:16,fontFamily:'Roboto',marginRight:15}}>{this.props.hotelreview} </Text>  : null}
...
<View style={{backgroundColor:"white",height:100,width:'100%'}} />
  ...
  <View style={{flexDirection:'row', borderTopColor: '#ecf0f1',
          ...
          multiline={true}
            ...
            />
   <TouchableOpacity style={{paddingTop:15,marginRight:10}} onPress={this.comment.bind(this, this.props.hotelkey) }>
<Text>Post</Text>
  </TouchableOpacity>        
 </View>
...
export default connect(mapStateToProps,{ signOut })(HotelDetail);