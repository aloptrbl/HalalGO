...
class ProductDetail extends React.Component {
    ...
    ratingCompleted(rating) {
      const userlikesRef = database.ref('rating').child(`${this.props.productkey}/${this.props.user.uid}/`);
      const commentsRef = database.ref('rating').child(`${this.props.productkey}/ratingscount`);
    ...
    componentDidMount() {
      database.ref('rating').child(`${this.props.productkey}/${this.props.user.uid}/`).on('value', (snap) =>{
        snap.forEach((child) => {
          this.setState({
        rating: child.val(),
          })
      ...
      database.ref('rating').child(`${this.props.productkey}`).on('value', (snap) =>{
      let total = 0;
      let user = 0;
      let rating = 0;
        snap.forEach((child) => {
      ...
      database.ref('comments').child(`${this.props.productkey}`).on('value', (snaps) => {
        var item = [];
        var commentkey = [];
...
<Text h4 style={{marginTop:15,alignSelf:'center',fontSize:16}}> {this.props.productname} </Text>
{this.props.productsupplier ? <Text  style={{marginTop:2,alignSelf:'center',fontSize:12}}>Supplier: {this.props.productsupplier} </Text> : null}
{this.props.productstate ? <Text  style={{marginTop:2,alignSelf:'center',fontSize:14}}> {this.props.productstate},{this.props.productcountry} </Text> : null}
...
<Text h4 style={{marginTop:15,fontSize:20,marginLeft:15}}> Category  </Text>
{this.props.productcategory ? <Text  style={{marginTop:2,fontSize:16,marginLeft:15}}> {this.props.productcategory} </Text> : null}
<Text h4 style={{marginTop:15,fontSize:20,marginLeft:15}}> Price  </Text>
{this.props.productprice ? <Text  style={{marginTop:2,fontSize:16,marginLeft:16,fontFamily:'Roboto',marginRight:15}}>RM {this.props.productprice} </Text>  : null}
<Text h4 style={{marginTop:15,fontSize:20,marginLeft:15}}> Review  </Text>
{this.props.productreview ? <Text  style={{marginTop:2,fontSize:16,marginLeft:16,fontFamily:'Roboto',marginRight:15}}>{this.props.productreview} </Text>  : null}
...
<View style={{backgroundColor:"white",height:100,width:'100%'}} />
  ...
  <View style={{flexDirection:'row', borderTopColor: '#ecf0f1',
          ...
          multiline={true}
            ...
            />
   <TouchableOpacity style={{paddingTop:15,marginRight:10}} onPress={this.comment.bind(this, this.props.productkey) }>
<Text>Post</Text>
  </TouchableOpacity>        
 </View>
 ...
 <TouchableOpacity 
        ...
        onPress={Actions.pop } >
          <Ionicons name="ios-arrow-round-back" size={45} style={{color: 'white',  textShadowColor: 'rgba(0, 0, 0, 0.75)',
...
export default connect(mapStateToProps,{ signOut })(ProductDetail);