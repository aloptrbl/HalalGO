...
class RestaurantPost extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            user: '',
   ...
   />
           ...
           <View style={{flexDirection:'row',alignContent:'center'}}>
                 <FastImage style={{height:50,width:50,borderRadius:50,marginTop:10,marginLeft:10}} source = {{ uri: this.props.submitimage,  priority: FastImage.priority.high }}/>  
         ...
         <TouchableOpacity  style={{flex:1,justifyContent:'center',flexDirection:'row',marginLeft:10}}><Ionicon name={this.props.postid  ? 'ios-heart' : 'ios-heart-outline'} style={{color: this.props.postid ? 'red' : 'white',fontSize:23}}/><Text style={{color:'white',fontSize:15, alignItems: 'center',alignSelf:'center'}}> {this.props.likescount} Likes</Text></TouchableOpacity>
            ...
            <Text style={{fontSize:15,color:'black',fontWeight:'600'}}>{this.props.submitfullname} </Text>
...
export default connect(mapStateToProps,{ signOut })(RestaurantPost);